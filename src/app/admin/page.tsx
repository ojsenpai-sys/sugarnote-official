...
export default async function AdminDashboard() {
  ...
  return (
    <>
      ...
      <p className="text-slate-500 mt-2">{siteConfig.name} 公式サイトの管理画面へようこそ！左側のメニューから各コンテンツを編集できます。</p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mt-8">
        {stats.map((stat, i) => (
          <div key={i} className="bg-white rounded-2xl p-6 shadow-sm border border-slate-100 flex flex-col items-center justify-center">
            ...
            <p className="text-5xl font-bold text-primary-500 mt-4">{stat.value}</p>
          </div>
        ))}
      </div>
    </>
